...
const MatchpointPage: React.FC = () => {
    ...
    const [eventKeyInput, setEventKeyInput] = useState<string>(eventKeyFromUrl || '2025mxle');
    ...
    // El handler del formulario ahora solo navega a la nueva URL.
    const handleFormSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (eventKeyInput) {
            navigate(`/matchpoint/event/${eventKeyInput}`);
        }
    };
        ...
        <main className="min-h-screen w-full pt-32 pb-16 px-4 md:px-8 font-['Poppins']">
            <div className="container mx-auto max-w-7xl text-center">
                ...
                <p className="max-w-4xl mx-auto mb-10 text-lg md:text-xl text-text-muted font-light">
                    Enter an FRC event key to fetch predictions and click on a match to see a detailed breakdown.
                </p>
                ...
                <form onSubmit={handleFormSubmit} className="max-w-2xl mx-auto">
                    <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        />
                        <button type="submit" className="flex justify-center items-center bg-accent text-white font-bold text-lg px-8 py-3 rounded-lg hover:bg-opacity-80 transition-all disabled:bg-gray-600 disabled:cursor-not-allowed">
                            Analyze Event
                        </button>
                    </div>
                </form>
                                    ...
                                    <tbody className="divide-y divide-border">
                                        {predictions.map((p) => (
                                            ...
                                            <tr 
                                                ...
                                                onClick={() => navigate(`/matchpoint/match/${p.match_key}`)}
                                            >
                                                {p.status === 'played' 
                                                    ? <PlayedMatchRow p={p} />
...
const UpcomingMatchRow: React.FC<{ p: Prediction }> = ({ p }) => {
    return (
        <>
            ...
            <td className="py-3 px-4 font-mono text-center bg-red-500/10 text-red-300">{p.predicted_scores.red}</td>
            <td className="py-3 px-4 font-mono text-center bg-blue-500/10 text-blue-300">{p.predicted_scores.blue}</td>
            <td className="py-3 px-4 font-mono text-center font-bold bg-red-500/20 text-red-300">-</td>
            <td className="py-3 px-4 font-mono text-center font-bold bg-blue-500/20 text-blue-300">-</td>
            <td className="py-3 px-4 text-center">
