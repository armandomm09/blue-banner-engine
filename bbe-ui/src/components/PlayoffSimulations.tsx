...
export const PlayoffSimulations: React.FC<PlayoffSimulationsProps> = ({
  eventKey,
  actualWinnerTeams,
}) => {
  const [simulationsData, setSimulationsData] = useState<SimulationData | null>(
    null
  );
  ...
  const runSimulation = async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/predict/event/${eventKey}/playoff/${count}`
      );
      if (!response.ok) {
  ...
  if (
    ...
    console.log(
      ...
      predictedWinnerAlliance.teams.sort()
    );
    console.log("Actual Winner Teams:", actualWinnerTeams.sort());
    const isCorrect =
      JSON.stringify(predictedWinnerAlliance.teams.sort()) ===
      JSON.stringify(actualWinnerTeams.sort());
      ...
      <div className="min-h-[200px]">
        ...
        {!isLoading && !error && !simulationsData && (
          <div className="text-center py-10 text-text-muted">
            <p>
              Enter the number of simulations and click "Run" to see the
              results.
            </p>
          </div>
        )}
          ...
          <div className="animate-fade-in">
            ...
            {predictionCheck && (
              <div className="mb-4 p-3 rounded-lg border border-border text-center">
                ...
                <span
                  className={`px-3 py-1 text-base font-bold rounded-full ${predictionCheck.className}`}
                >
                  {predictionCheck.text}
                </span>
                <p className="text-xs text-text-muted mt-1">
                  {predictionCheck.actualWinner}
                </p>
              </div>
            )}
            ...
            <div className="space-y-3">
              {sortedResults.map((result) => (
                <div
                  key={result.alliance_number}
                  className="bg-background/20 p-3 rounded-lg"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                    <div className="w-full sm:w-1/3">
                      <p className="font-bold text-white">
                        Alliance {result.alliance_number}
                      </p>
